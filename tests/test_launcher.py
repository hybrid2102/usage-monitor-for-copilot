...
Tests for the ``usage-monitor-for-copilot`` shell launcher in the project root.
...
LAUNCHER = Path(__file__).resolve().parent.parent / 'usage-monitor-for-copilot'
...
class TestLauncherInvocation(unittest.TestCase):
    ...
    def test_runs_the_package_with_the_checkout_interpreter(self):
        ...
        self.assertIn('args=-m usage_monitor_for_copilot', result.stdout)
    ...
    def test_passes_arguments_through(self):
        ...
        self.assertIn('args=-m usage_monitor_for_copilot --config-dir=/tmp/example', result.stdout)
