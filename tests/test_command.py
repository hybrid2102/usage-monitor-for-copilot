...
from usage_monitor_for_copilot import command
from usage_monitor_for_copilot.platforms import no_window_kwargs
from usage_monitor_for_copilot.command import run_event_command
...
class TestRunEventCommand(unittest.TestCase):
    ...
    @patch('usage_monitor_for_copilot.command.subprocess.Popen')
    ...
    @patch('usage_monitor_for_copilot.command.traceback.print_exc')
    @patch('usage_monitor_for_copilot.command.subprocess.Popen', side_effect=OSError('not found'))
    ...
    @patch('usage_monitor_for_copilot.command.traceback.print_exc')
    @patch('usage_monitor_for_copilot.command.subprocess.Popen', side_effect=ValueError('bad'))
    ...
    @patch('usage_monitor_for_copilot.command.subprocess.Popen')
    @patch('usage_monitor_for_copilot.command.sys')
...
class TestRunEventCommandCaptureOutput(unittest.TestCase):
    ...
    @patch('usage_monitor_for_copilot.command.threading.Thread')
    @patch('usage_monitor_for_copilot.command.subprocess.Popen')
    ...
    @patch('usage_monitor_for_copilot.command.show_error_box')
...
class TestStartupFailureWindow(unittest.TestCase):
    ...
    def _run(self, returncode: int, runtime: float, report_late_failures: bool):
        ...
        with (
            patch('usage_monitor_for_copilot.command.show_error_box') as mock_box,
            patch('builtins.print') as mock_print,
            patch('usage_monitor_for_copilot.command.threading.Thread') as mock_thread,
            patch('usage_monitor_for_copilot.command.time.monotonic', fake_monotonic),
            patch('usage_monitor_for_copilot.command.subprocess.Popen') as mock_popen,
        ):
            mock_popen.return_value.communicate.return_value = ('', 'the failure detail')
