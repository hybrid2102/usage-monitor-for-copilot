...
from usage_monitor_for_codex import command
...
class TestRunEventCommandCaptureOutput(unittest.TestCase):
    ...
    @patch('usage_monitor_for_codex.command.threading.Thread')
    @patch('usage_monitor_for_codex.command.subprocess.Popen')
    ...
    @patch('usage_monitor_for_codex.command.show_error_box')
...
class TestStartupFailureWindow(unittest.TestCase):
    ...
    def _run(self, returncode: int, runtime: float, report_late_failures: bool):
        ...
        with (
            patch('usage_monitor_for_codex.command.show_error_box') as mock_box,
            patch('builtins.print') as mock_print,
            patch('usage_monitor_for_codex.command.threading.Thread') as mock_thread,
            patch('usage_monitor_for_codex.command.time.monotonic', fake_monotonic),
            patch('usage_monitor_for_codex.command.subprocess.Popen') as mock_popen,
        ):
            mock_popen.return_value.communicate.return_value = ('', 'the failure detail')
